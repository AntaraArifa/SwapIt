...
const CourseStudents = ({ courseId,onMessageClick }) => {
  ...
  const fetchCourseStudents = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/courses/students/${courseId}`,
        {
          method: "POST",
          headers: headers,
          credentials: "include",
  ...
  const handleApproveRegistration = async (registrationId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/courses/status/update`,
        {
          method: "POST",
          headers: headers,
          credentials: "include",
  ...
  const handleCompleteRegistration = async (registrationId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/courses/status/complete`,
        {
          method: "POST",
          headers: headers,
          credentials: "include",
  ...
  const getTabCounts = () => {
    const counts = {
      ...
      pending: students.filter(
        (s) => (s.courseStatus || s.status) === "pending"
      ).length,
      registered: students.filter(
        (s) => (s.courseStatus || s.status) === "registered"
      ).length,
      completed: students.filter(
        (s) => (s.courseStatus || s.status) === "completed"
      ).length,
    };
    return counts;
  };
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center space-x-3">
                          {getStatusBadge(
                            registration.courseStatus || registration.status
                          )}
                          {/* Action buttons based on status */}
                          {(registration.courseStatus === "pending" ||
                          ...
                          {(registration.courseStatus === "registered" ||
                            ...
                            <button
                              onClick={() =>
                                handleCompleteRegistration(registration._id)
                              }
                          ...
                          <button
                            ...
                            title="Send Message"
                            onClick={() => {
                              onMessageClick(registration.studentId);
                            }}
                          >
