  ...
  const fetchSessions = async () => {
    try {
      ...
      const uniqueListingIds = [
        ...new Set(
          response.map((s) => s.skillListingID?._id).filter((id) => !!id)
        ),
      ];
      const slotsMap = {};
      await Promise.all(
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <option value="">--:-- --</option>
                      {(
                        availableSlotsMap[session.skillListingID?._id] || []
                      ).map((slot) => (
              ...
              {session.status === "accepted" && (
                <div className="flex gap-2 mt-2">
                  ...
                  {hasSessionPassed(session.scheduledTime) && (
                    <button
                      className="bg-gray-800 text-white text-sm px-3 py-1 rounded-full hover:bg-gray-900"
                      onClick={() =>
